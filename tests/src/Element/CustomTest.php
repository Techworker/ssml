<?php
/**
 * Copyright (c) Zandura GmbH - all rights reserved.
 *
 * For the full copyright and license information, please view the LICENSE
 * file that was distributed with this source code.
 *
 * Written by Benjamin Ansbach <benjamin.ansbach@zandura.net>, 2016
 */
declare(strict_types = 1);

namespace Techworker\Ssml\Tests\Element;

use Techworker\Ssml\Element\Custom;
use Techworker\Ssml\StrictException;
use Techworker\Ssml\Tests\TestCase;

class CustomTest extends TestCase
{
    public function testFactory()
    {
        $element = Custom::factory('foo');
        self::assertEquals('<foo/>', (string)$element);
    }

    public function testFactoryWithAttributes()
    {
        $element = Custom::factory('foo', ['test1' => 'ABC', 'test2' => 'DEF']);
        self::assertEquals('<foo test1="ABC" test2="DEF"/>', (string)$element);
    }
}